...
> **Nota (2026):** Pollinations migró a un sistema de créditos «pollen»; el acceso **anónimo** ya no genera imágenes nuevas (error *«Insufficient balance»*). Sigue siendo **cero costo**: regístrate gratis en [auth.pollinations.ai](https://auth.pollinations.ai), crea un token y ponlo como `POLLINATIONS_TOKEN` en `.env` (los grants diarios gratis cubren `flux`, sin tarjeta). El flujo recomendado —copiar JSON/packs a un chatbot free— **no** necesita esto.

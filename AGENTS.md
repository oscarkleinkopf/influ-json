...
- **Face-anchor vs JSON:** `POST /api/personas/:id/variants` ancla a `persona.image` solo si no es stub (`nano_banana_*` / placeholders) y no contradice tez/pelo del `character_lock` (`anchor-lock-consistency.js`). Si hay mismatch (caso Eru: JSON morena + foto rubia), genera solo con IDENTITY LOCK de texto. Reiniciar el server tras pull para cargar el guard.

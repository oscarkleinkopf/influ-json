...
- **Face-anchor vs JSON:** `POST /api/personas/:id/variants` ancla a `persona.image`. Stubs `nano_banana_*` no se anclan. Si el retrato es **inspiración** (`assets/references/ref_*` / `inspired_from_photo`), la foto gana: se sincroniza tez/pelo/ojos del lock y se mantiene img2img. Si el mismatch es accidental (scratch sin inspiración), se salta el ancla y gana el JSON. **Importante:** `flux` es text-only — con referencia el server usa `nanobanana` (`POLLINATIONS_IMG2IMG_MODEL`). `resolveFraming` respeta `framing` explícito (no bajar a fullbody y perder el ancla). Reiniciar server tras pull.

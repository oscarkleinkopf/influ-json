...
- **Git credentials (P0 harness):** el remote debe ser URL limpia sin secretos, p. ej. `https://github.com/oscarkleinkopf/influ-json.git`. Auth vía credential helper (`gh auth git-credential`), **nunca** `https://x-access-token:…@github.com/…` en `remote.origin.url` ni en `url.*.insteadOf` de `~/.gitconfig`. Si `git remote get-url origin` muestra un token, quitar los `insteadOf` con token y volver a `git remote set-url origin https://github.com/oscarkleinkopf/influ-json.git`. Validar: `git remote get-url origin` sin credencial + `git ls-remote origin` OK. El backup del studio (`ENABLE_GIT_BACKUP=1` → `git push origin main` en `runGitBackup`) depende de ese helper; no re-embeber tokens en la URL para “arreglar” el push.

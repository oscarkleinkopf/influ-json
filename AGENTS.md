...
- **Fotorrealismo (bocetos Pollinations):** toggle «Más fotorrealismo» en el vault (default on) → `photoQuality=high` añade `PHOTO QUALITY LOCK` y usa modelo `zimage` (~0.004 pollen) en gens **sin** referencia y **no** spicy/lingerie (ahí se queda en `flux` por fiabilidad NSFW). Override: `POLLINATIONS_PHOTO_MODEL`. Img2img (face-anchor): `POLLINATIONS_IMG2IMG_MODEL` default `nanobanana`. Path free chatbot no cambia.
- **Spicy gen:** el framing explícito del vault (`medium`/`fullbody`) no debe ser pisado por texto del prompt; si Spicy falla con 503, revisar que el server se reinició tras pull (fallback flux ya está en `ai-service`).
- **Inspiración caucásica / rubia:** si la foto es `assets/references/ref_*`, al generar variante se sincroniza el lock desde la foto (`enrichDetailedFromInspiration`). Etnia fair+blonde → `Caucásica / Europea de tez clara` (evitar «Latina», que sesga a pelo oscuro). Reiniciar server tras pull.

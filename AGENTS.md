...
- Arranque / tests: ver README + `package.json` (`npm start`, `npm test` → DB+uploads aislados, `npm run smoke`, `npm run layout-smoke` Chrome). Auth: `STUDIO_PIN` en `.env`.
...
- **Diagnostics (runtime):** `GET /api/status` — auth/bind/PIN/setup + flags de image providers. Sin sesión: **no** incluye `dataDir`/`dbPath` ni URLs internas de providers (recon slim; ver `docs/SECURITY_MARKET.md`). Con sesión: paths + caps completos. `GET /api/queue-status` — **requiere auth** (sesión o Bearer); cola (`pendingCount`, cooldown 429, `currentTaskInfo`); úsalo cuando gen se atasca o sospechas cooldown anti-429.
...
- **CSRF:** mutaciones cookie exigen `X-CSRF-Token` (= `session.csrfToken`; login/`/api/status`/`/api/auth/me` lo devuelven). Bearer exento. Apagar: `CSRF_PROTECTION=0`. Guía: `docs/SECURITY_MARKET.md`.

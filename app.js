...
async function refreshProfilesSettingsList() {
  ...
  try {
    ...
    if (cuentaSummary && current) {
      ...
      cuentaSummary.innerHTML = '<p style="font-size:12px;color:var(--text-muted);margin:0;">Sin perfil activo.</p>';
    }
    ...
    const renderRows = (container, profiles, { allowDelete }) => {
      ...
      <div class="profile-row ${p.id === currentId ? 'is-current' : ''}">
        ...
        <div class="profile-row-actions">
          ...
          ${(allowDelete && p.id !== currentId) ? `<button type="button" class="btn btn-secondary btn-sm" data-delete-profile="${p.id}" style="color:var(--danger);">Eliminar</button>` : ''}
        </div>
      </div>
  ...
  filtered.forEach(p => {
    ...
    card.innerHTML = `
      <div class="portfolio-card-img-wrapper">
        ...
        ${!isArchivedPersona(p) ? `<button type="button" class="portfolio-badge ${exportBadgeClass}" data-export-status="${exportStatus.kind}" title="${exportStatus.summary || exportStatus.label}">${exportStatus.label}</button>` : ''}

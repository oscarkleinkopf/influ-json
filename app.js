...
function initImportModal() {
  ...
  let lastImportImagePaths = []; // refs subidas en preview → borrar al descartar
  ...
  /** Descartar preview: no deja huérfanos en roster ni refs en disco */
  async function discardImportPreview() {
    const pathsToClean = lastImportImagePaths.slice();
  ...
  if (btnAnalyze) {
    btnAnalyze.addEventListener('click', async () => {
      ...
      try {
        ...
        lastImportImagePaths = Array.isArray(data.imagePaths) ? data.imagePaths.slice() : [];
        if (!lastImportImagePaths.length && lastImportedPersona?.image) {
          lastImportImagePaths = [lastImportedPersona.image];
        }
  ...
  if (btnConfirm) {
    btnConfirm.addEventListener('click', async () => {
      ...
      try {
        ...
        importIsPreview = false;
        lastImportImagePaths = []; // confirmados: no borrar refs

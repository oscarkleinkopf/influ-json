...
async function sampleImageAppearance(absolutePath) {
  ...
  // Drop near-black furniture/shadows so dark wood behind honey-blonde curls
  // (Eru-style ref) does not dominate and classify as "dark hair".
  const hairPool = topNonSkin.concat(sideHair).filter((p) => p.bright >= 48);
  if (hairPool.length >= 10) {
    ...
    const honeyVotes = hairPool.filter(
      (p) => p.warm > 18 && p.bright >= 48 && p.bright < 175
    ).length;
    const darkVotes = hairPool.filter((p) => p.bright < 58 && p.warm < 22).length;
    if (
      blondeVotes > hairPool.length * 0.12 ||
      honeyVotes > hairPool.length * 0.16 ||
      (avgB >= 95 && avgW > 16) ||
      // Absolute honey count: furniture-heavy backgrounds dilute ratios
      (skin === 'fair' && honeyVotes >= 22)
    ) {
      hair = 'blonde';
    } else if (darkVotes > hairPool.length * 0.45 || (avgB < 65 && avgW < 18)) {
      hair = 'dark';
    }
  }
...
function appearanceLabelsFromSample(sample) {
  ...
  if (sample.skin === 'fair') {
    ...
    // NO "Latina" — image models associate Latina with dark hair and override
    // a fair Caucasian / honey-blonde reference photo.
    out.ethnicity_hint = 'Caucásica / Europea de tez clara';
    out.skin_lock = 'fair light porcelain-beige skin, Caucasian, NOT dark, NOT morena, NOT Latina-default dark hair';
    out.skin_avoid = 'dark skin, deep tan, morena, black hair, dark brown hair';
...
async function enrichDetailedFromInspiration(imagePath, detailedJSON, opts = {}) {
  ...
  // Fair Caucasian ref: drop "Latina" (models → dark hair) even if form said Latina
  if (
    labels.ethnicity_hint &&
    (sample?.skin === 'fair' || sample?.hair === 'blonde') &&
    (!base.identity.ethnicity_appearance
      || /latina/i.test(String(base.identity.ethnicity_appearance))
      || /europea\s*\/\s*latina/i.test(String(base.identity.ethnicity_appearance)))
  ) {
  ...
  if (labels.hair_color && sample?.hair === 'blonde') {
    ...
    base.hair.color_hex = '#d4b483';
  } else if (
    sample?.hair === 'dark' &&
    /rubio|blonde/i.test(String(base.hair.color || '')) &&
    sample?.skin === 'fair'
  ) {
    // Keep existing blonde lock when furniture-dark sample falsely says dark hair
    base.hair.color_hex = '#d4b483';
  }
  // Blonde label must never keep dark-brown hex (#3d2314) — that poisons prompts
  if (/rubio|blonde|platino/i.test(String(base.hair.color || ''))) {
    if (!base.hair.color_hex || /#?3d2314|#?2c1810|#?1a1a1a/i.test(String(base.hair.color_hex))) {
      base.hair.color_hex = '#d4b483';
    }
  }
  ...
  // Fair + blonde/inspire: warm lighting often mis-labels light eyes as brown — prefer light cue
  if (
    sample?.skin === 'fair' &&
...
function reconcilePromptWithInspiration(prompt, detailedJSON) {
  ...
  if (FAIR_SKIN_RE.test(f.skin_tone || '') || /NOT dark|porcelain|beige claro/i.test(f.skin_lock || '')) {
    ...
    p = p.replace(/\bcolor Negro\b/gi, `color ${h.color || 'Rubio dorado'}`);
    // Strip Latina ethnicity bias when inspiration is fair Caucasian / blonde
    if (/rubio|blonde|cauc[aá]sic|europea de tez clara|porcelain/i.test(`${h.color} ${f.skin_tone} ${d.identity?.ethnicity_appearance || ''}`)) {
      p = p.replace(/\bLatina de tez clara\b/gi, 'Caucásica / Europea de tez clara');
      p = p.replace(/\bEuropea\s*\/\s*Latina de tez clara\b/gi, 'Caucásica / Europea de tez clara');
      p = p.replace(/\bLatina\b/gi, 'Caucásica');
    }
  }
  const reinforce = [
...
/**
 * Build DB column `hair` / `ethnicity` from enriched detailedJSON so roster
 * cards and hairFallback match the inspiration photo (not stale "Latina"/"Negro").
 */
function personaFieldsFromDetailed(persona, detailedJSON) {
  const d = parseDetailed(detailedJSON);
  const h = d.hair || {};
  const eth = d.identity?.ethnicity_appearance || persona?.ethnicity || null;
  const hairParts = [
    h.length,
    h.texture,
  ...
  return {
    ethnicity: eth || persona?.ethnicity,
    hair: hairParts.length ? hairParts.join(', ') : (persona?.hair || null)
  };
}
